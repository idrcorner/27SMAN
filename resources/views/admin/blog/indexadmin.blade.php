...
@section('content')
        ...
        <div class="panel-header bg-primary-gradient">
            <div class="page-inner py-5">
                <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                    <div>
                        <h2 class="text-white pb-2 fw-bold">Blog Guru</h2>
                        
                    </div>
                    <div class="ml-md-auto py-2 py-md-0">                  
                        {{-- <a href="{{route('blog.create')}}" class="btn btn-secondary btn-round">Tambah Artikel</a> --}}
                    </div>
                </div>
            </div>
        </div>
                           ...
                           <table class="table table-bordered table-hovered" id='iniTabel'>
                               ...
                               <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('blogadmin.show',$inf->id)}}">
                                        {{$inf->judul}}
                                        </a>
                                    </td>
                                    ...
                                    <td class="{{$inf->status==0?'text-warning':'text-primary'}}">
                                        <a href="{{route('statusblog',$inf->id)}}">
                                        {{$inf->status==0?'Arsip':'Publis'}}
                                    </a>
                                    </td>
                               <td>
                                        <a href="{{route('blogadmin.show',$inf->id)}}"><i class=" fas fa-search text-primary mr-2" title="Lihat Detail"></i></a>
                                       {{-- <a href="{{route('blog.edit',$inf->id)}}"> <i class=" fas fa-pen text-warning mr-2" title="Ubah"></i></a>
    ...
    <form id="deleteform" action="{{route('blog.delete')}}" method="post">
        @csrf
        @method('DELETE')
        <input id="idBlog" type="hidden" name="id" value="">
